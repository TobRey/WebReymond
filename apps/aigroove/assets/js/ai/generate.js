...
import { activeProvider, getProvider, AIError, analysePrompt } from './providers.js';
...
/**
 * Sinnvolle One-Shot-Laenge.
 * Die KI leitet sie aus Klangart und gewuenschtem Ausklang selbst ab –
 * eine "kurze Hi-Hat" bekommt so von allein weniger Zeit als ein "langes Pad".
 */
function oneShotSeconds(prompt) {
  return clamp(analysePrompt(prompt).seconds, 0.05, 30);
}
...
export async function generateVariants({
  ...
  if (inputSampleId) {
    ...
    if (!provider.capabilities.audioToAudio) {
      throw new AIError('unsupported', `${provider.label} kann vorhandene Samples nicht weiterverarbeiten.`);
    }
