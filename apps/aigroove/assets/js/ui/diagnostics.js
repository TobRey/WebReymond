...
import { engineStatus, activeProvider } from '../ai/providers.js';
...
export async function openDiagnostics() {
  ...
  aiSection.appendChild(row('Engine', activeProvider().label));
  aiSection.appendChild(row('Version', engine.engine));
  aiSection.appendChild(row('Rechenweg', engine.worker));
  aiSection.appendChild(row('Rechenqualität', engine.quality));
  aiSection.appendChild(row('Internet nötig', 'nein'));
  aiSection.appendChild(row('API-Key', 'nicht erforderlich'));
