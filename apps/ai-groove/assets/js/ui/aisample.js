...
export function openAiGenerateDialog(prefill = '') {
  ...
  providerRow.appendChild(
    h('div.settings-row__label', null, [
      h('b', { text: `Anbieter: ${provider.label}` }),
      h('span', { text: provider.description }),
    ]),
  );
  const connectBtn = h('button.btn.btn--sm', { type: 'button', text: 'KI verbinden' });
  ...
  body.appendChild(
    h('div.field', null, [
      ...
      promptInput,
      h('p.field__hint', {
        text: 'Je konkreter, desto besser: Instrument, Charakter, Länge, Tempo. Englisch funktioniert bei den meisten Anbietern am besten.',
      }),
    ]),
  );
  ...
  body.appendChild(
    h('div.settings-row', null, [
      h('div.settings-row__label', null, [
        h('b', { text: `Anbieter: ${provider.label}` }),
        h('span', {
          text: provider.capabilities.audioToAudio
            ? 'Das vorhandene Sample wird als Ausgangsmaterial mitgesendet.'
            : 'Dieser Anbieter kann keine vorhandenen Samples weiterverarbeiten – es wird der lokale Generator verwendet.',
        }),
      ]),
    ]),
  );
