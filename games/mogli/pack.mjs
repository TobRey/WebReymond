// Packt games/mogli/web/ in eine ZIP-Datei, die man auf jeden Webspace
  ...
  if (!ALLOWED_NAMES.has(name) && !ALLOWED_EXTENSIONS.has(extension)) {
    throw new Error(
      `${path}: Endung "${extension || name}" ist nicht vorgesehen. ` +
        'Mogli kommt ohne Binärdateien aus – wer das ändern will, erweitert ' +
...
function main() {
  ...
  const versioned = join(OUT_DIR, `mogli-${version}.zip`);
  const stable = join(OUT_DIR, 'mogli.zip');
  writeFileSync(versioned, zip);
  writeFileSync(stable, zip);

  const rawTotal = entries.reduce((sum, e) => sum + e.raw.length, 0);
  console.log(`Mogli ${version}\n`);
