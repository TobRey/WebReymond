...
preg_match('/name="csrf" value="([a-f0-9]+)"/', $b, $m);
$csrf = $m[1] ?? '';
check($csrf !== '', 'Admin-Seite liefert CSRF-Token');
...
[$s, $b] = http('POST', $BASE . 'admin.php', $form(['csrf' => 'bad', 'do' => 'setup']), ['Content-Type: application/x-www-form-urlencoded'], $jar);
check(str_contains($b, 'Sitzung abgelaufen'), 'Admin-Formular ohne gültiges CSRF-Token abgelehnt');
