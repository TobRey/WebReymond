...
// (siehe games/heavenclimb/test/physics.test.mjs).
//
 ...
 * @param {{ dropThrough?: boolean }} [options]
 ...
 */
export function moveAndCollide(body, level, options = {}) {
  const dropThrough = options.dropThrough === true;
  const result = {
    hitLeft: false,
    hitRight: false,
    hitTop: false,
  ...
  for (let s = 0; s < steps; s += 1) {
    ...
    moveY(body, level, stepY, result, dropThrough);
  }
...
function moveY(body, level, dy, result, dropThrough) {
  if (dy === 0) return;
  ...
  if (dy > 0) {
    ...
    for (let col = col0; col <= col1; col += 1) {
      ...
      const blocking =
        isSolid(tile) ||
        // Durchstiegsplattform: hält nur von oben, und nur wenn man nicht
        // bewusst hindurchfallen will.
        (tile === T.ONEWAY && !dropThrough && bottomBefore <= row * TILE);
      if (blocking) {
