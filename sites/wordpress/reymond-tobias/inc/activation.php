...
function rt_after_switch_theme() {
	...
	update_option( 'blogdescription', __( 'DJ · Schweiz', 'reymond-tobias' ) );
	flush_rewrite_rules();
}
...
function rt_create_pages() {
	$blueprint = array(
		'start'   => array(
			'title'    => __( 'Start', 'reymond-tobias' ),
			'template' => '',
			'order'    => 1,
			'content'  => rt_block_paragraphs(
				array(
					'<strong>Reymond Tobias</strong> legt auf, was um zwei Uhr morgens noch funktioniert: harte Kicks, weite Flächen, ein Aufbau, der nicht loslässt. Kein Set gleicht dem anderen – gelesen wird der Raum, nicht die Playlist.',
		...
		'kontakt' => array(
			'title'    => __( 'Kontakt', 'reymond-tobias' ),
...
function rt_create_menu( $pages ) {
	$name = __( 'Hauptmenü', 'reymond-tobias' );
...
function rt_seed_gigs() {
	...
	$gigs = array(
		array( __( 'Clubnacht', 'reymond-tobias' ), 'TBA', __( 'Ort folgt', 'reymond-tobias' ), __( 'Beispiel', 'reymond-tobias' ) ),
		array( __( 'Festival', 'reymond-tobias' ), 'TBA', __( 'Ort folgt', 'reymond-tobias' ), __( 'Beispiel', 'reymond-tobias' ) ),
		array( __( 'Private Feier', 'reymond-tobias' ), 'TBA', __( 'Auf Anfrage', 'reymond-tobias' ), __( 'Beispiel', 'reymond-tobias' ) ),
	);
...
function rt_admin_notice() {
	...
	printf(
		...
		esc_html__( 'Reymond Tobias:', 'reymond-tobias' ),
		esc_html(
			sprintf(
				/* translators: %d: Anzahl */
				_n( '%d Titel hat noch keine Audiodatei – der Player zeigt dafür den Vorschaumodus.', '%d Titel haben noch keine Audiodatei – der Player zeigt dafür den Vorschaumodus.', $missing, 'reymond-tobias' ),
				$missing
			)
		),
		esc_url( admin_url( 'edit.php?post_type=rt_track' ) ),
		esc_html__( 'Titel bearbeiten', 'reymond-tobias' )
	);
}
