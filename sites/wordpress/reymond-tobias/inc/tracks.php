...
function rt_register_track() {
	register_post_type(
		...
		array(
			'labels'        => array(
				'name'               => __( 'Titel', 'reymond-tobias' ),
				'singular_name'      => __( 'Titel', 'reymond-tobias' ),
				'add_new'            => __( 'Titel hinzufügen', 'reymond-tobias' ),
				'add_new_item'       => __( 'Neuer Titel', 'reymond-tobias' ),
				'edit_item'          => __( 'Titel bearbeiten', 'reymond-tobias' ),
				'all_items'          => __( 'Alle Titel', 'reymond-tobias' ),
				'menu_name'          => __( 'Musik', 'reymond-tobias' ),
				'not_found'          => __( 'Noch keine Titel angelegt.', 'reymond-tobias' ),
			),
			'public'        => false,
			'show_ui'       => true,
			'show_in_menu'  => true,
...
function rt_track_meta_box() {
	add_meta_box(
		'rt_track_details',
		__( 'Angaben zum Titel', 'reymond-tobias' ),
...
function rt_track_meta_box_html( $post ) {
	...
	?>
	<p>
		<label for="rt_tag"><strong><?php esc_html_e( 'Stilrichtung', 'reymond-tobias' ); ?></strong></label><br />
		<input type="text" id="rt_tag" name="rt_tag" class="widefat" value="<?php echo esc_attr( $tag ); ?>"
			placeholder="<?php esc_attr_e( 'z. B. Peak Time Techno', 'reymond-tobias' ); ?>" />
	</p>
	<p>
		<label for="rt_duration"><strong><?php esc_html_e( 'Dauer', 'reymond-tobias' ); ?></strong></label><br />
		<input type="text" id="rt_duration" name="rt_duration" value="<?php echo esc_attr( $duration ); ?>"
			placeholder="6:12" />
		<span class="description"><?php esc_html_e( 'Wird angezeigt, bis die Audiodatei geladen ist.', 'reymond-tobias' ); ?></span>
	</p>
	<p>
		<label for="rt_audio"><strong><?php esc_html_e( 'Audiodatei', 'reymond-tobias' ); ?></strong></label><br />
		<input type="url" id="rt_audio" name="rt_audio" class="widefat" value="<?php echo esc_url( $audio ); ?>"
			placeholder="https://…/set.mp3" />
		<button type="button" class="button" id="rt_audio_pick"><?php esc_html_e( 'Datei wählen', 'reymond-tobias' ); ?></button>
		<span class="description"><?php esc_html_e( 'Fehlt die Datei, läuft der Player für diesen Titel im Vorschaumodus.', 'reymond-tobias' ); ?></span>
	</p>
	<script>
