...
function rt_handle_contact() {
	...
	$subject = sprintf(
		...
		__( 'Booking-Anfrage: %1$s – %2$s', 'reymond-tobias' ),
		$anlass,
		$name
	);
	...
	$body = implode(
		"\n",
		array(
			__( 'Name: ', 'reymond-tobias' ) . $name,
			__( 'E-Mail: ', 'reymond-tobias' ) . $email,
			__( 'Anlass: ', 'reymond-tobias' ) . $anlass,
			__( 'Datum: ', 'reymond-tobias' ) . ( $datum ? $datum : '–' ),
			__( 'Ort: ', 'reymond-tobias' ) . ( $ort ? $ort : '–' ),
			'',
			$nachricht ? $nachricht : __( '(keine Nachricht)', 'reymond-tobias' ),
			'',
			'—',
			__( 'Gesendet über ', 'reymond-tobias' ) . home_url( '/' ),
		)
	);
...
function rt_contact_message() {
	...
	switch ( $status ) {
		case 'ok':
			return __( 'Danke – die Anfrage ist unterwegs.', 'reymond-tobias' );
		case 'ungueltig':
			return __( 'Bitte Name, gültige E-Mail-Adresse und Anlass ausfüllen.', 'reymond-tobias' );
		case 'zuviel':
			return __( 'Es wurden zu viele Anfragen gesendet. Bitte später erneut versuchen.', 'reymond-tobias' );
		case 'fehler':
			return sprintf(
				/* translators: %s: E-Mail-Adresse */
				__( 'Versand fehlgeschlagen. Bitte direkt an %s schreiben.', 'reymond-tobias' ),
				rt_opt( 'email' )
			);
		default:
			return '';
