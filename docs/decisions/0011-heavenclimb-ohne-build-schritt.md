# 0011 – HeavenClimb als eigenständiger Ordner ohne Build-Schritt
...
## Entscheidung
1. Das Spiel liegt unter **`games/heavenclimb/`** und ist **kein Workspace-Paket**. Es hat kein
...
- **Kein `typecheck` für den Ordner.** Ohne `tsconfig.json` gibt es dort keine Typprüfung; die
  Absicherung leisten die Tests unter `games/heavenclimb/test/`.
